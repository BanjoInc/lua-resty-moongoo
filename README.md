...
#### insert
`<array>ids, <string or cbson.uint>error_or_number = colobj:insert(<array or table>docs, <nil or table>custom_params)`
Inserts new document(s) and returns their id's and number of inserted documents.

Optionally `custom_params` can be used to provide parameters like [`ordered`](https://docs.mongodb.com/manual/reference/method/db.collection.insert/#perform-an-unordered-inser) which by default is set to `true`.
